...
- モデル: `Salesforce/blip-image-captioning-base` でキャプション生成。
- タグ正規化: Edge Function 側で英語キーワードを日本語タグへマッピングし、最大3件返却。
- 失敗時: クライアントが固定候補 `["陶器", "地中海", "青色"]` へフォールバック。

## 実装内容

- `supabase/functions/suggest-tags/index.ts`
  - Base64 画像を受け取り、Hugging Face API に送信。
  - キャプションからタグ候補を抽出し、`{ tags: string[] }` を返却。
  - CORS とエラーレスポンスを実装。
