...
- モデル: `google/vit-base-patch16-224`（画像分類）+ `microsoft/trocr-base-printed`（OCR）+ DeepL 翻訳 API。
- 推論方法: 画像分類ラベル上位と OCR 文字列を組み合わせて DeepL へ渡し、短い日本語タグを最大3件返却。
...
- `supabase/functions/suggest-tags/index.ts`
  ...
  - OCR モデルで画像内文字列を抽出し、分類ラベルと合わせて DeepL で日本語タグ化する。
  - 配列/文字列/文字数/文体パターンを検証し、失敗時は固定候補へフォールバック。
  - `DEBUG_AI_TAGS=true` 時は `debug.imageLabels` / `debug.ocr` / `debug.deepl`（status/error/raw/入力/翻訳結果）を返却。
  - CORS とエラーレスポンスを実装。
...
- ローカル開発:
  ...
  - 任意で `DEBUG_AI_TAGS=true` を設定すると、レスポンスに `debug.imageLabels` / `debug.ocr` / `debug.deepl` を含める。
- 本番:
